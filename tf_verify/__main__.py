...
def get_tests(dataset, geometric, blur=None):
    if geometric:
        csvfile = open('../deepg/code/datasets/{}_test.csv'.format(dataset), 'r')
    elif blur:
        csvfile = open('../data/mnist_blur_{}shift.csv'.format(blur), 'r')
        print("Only the first 99 blur image samples are available.")
    else:
...
parser.add_argument('--blur', type=str, default=config.blur, help='value of blur can be either int, or float')
...
if config.vnn_lib_spec is not None:
    ...
    if dataset and config.input_box is None:
        tests = get_tests(dataset, config.geometric, blur)
    else:
